// // PreLoader.js
// import React from 'react';

// function PreLoader() {
//   return (
//     <div className="min-h-screen w-full bg-yellow-400 absolute overflow-hidden flex justify-center items-center">
//       <div className="pre-loader">
//         <svg
//           version="1.1"
//           id="L6"
//           xmlns="http://www.w3.org/2000/svg"
//           xmlnsXlink="http://www.w3.org/1999/xlink"
//           x="0px"
//           y="0px"
//           viewBox="0 0 100 100"
//           enableBackground="new 0 0 100 100"
//           xmlSpace="preserve"
//           className="w-16 h-16"
//         >
//           <rect
//             fill="none"
//             stroke="#fff"
//             strokeWidth="4"
//             x="25"
//             y="25"
//             width="50"
//             height="50"
//           >
//             <animateTransform
//               attributeName="transform"
//               dur="0.5s"
//               from="0 50 50"
//               to="180 50 50"
//               type="rotate"
//               id="strokeBox"
//               attributeType="XML"
//               begin="rectBox.end"
//             />
//           </rect>
//           <rect x="27" y="27" fill="#fff" width="46" height="50">
//             <animate
//               attributeName="height"
//               dur="1.3s"
//               attributeType="XML"
//               from="50"
//               to="0"
//               id="rectBox"
//               fill="freeze"
//               begin="0s;strokeBox.end"
//             />
//           </rect>
//         </svg>
//       </div>
//     </div>
//   );
// }

// export default PreLoader;



// PreLoader.js
import React from 'react';

function PreLoader() {
  return (
    <div className="relative w-full h-screen flex justify-center items-center overflow-hidden bg-yellow-400">
      {/* <video className="absolute top-0 left-0 w-full h-full object-cover z-0" autoPlay loop muted>
        <source src="/assets/videos/aot.mp4" type="video/mp4" />
        Your browser does not support the video tag.
      </video> */}
      <div className="relative z-10 flex justify-center items-center w-[30vw] h-[30vw]">
        <svg
          version="1.1"
          id="L6"
          xmlns="http://www.w3.org/2000/svg"
          xmlnsXlink="http://www.w3.org/1999/xlink"
          x="0px"
          y="0px"
          viewBox="0 0 100 100"
          enableBackground="new 0 0 100 100"
          xmlSpace="preserve"
          className="w-24 h-24"
        >
          <rect
            fill="none"
            stroke="#fff"
            strokeWidth="4"
            x="25"
            y="25"
            width="50"
            height="50"
          >
            <animateTransform
              attributeName="transform"
              dur="0.5s"
              from="0 50 50"
              to="180 50 50"
              type="rotate"
              id="strokeBox"
              attributeType="XML"
              begin="rectBox.end"
            />
          </rect>
          <rect x="27" y="27" fill="#fff" width="46" height="50">
            <animate
              attributeName="height"
              dur="1.3s"
              attributeType="XML"
              from="50"
              to="0"
              id="rectBox"
              fill="freeze"
              begin="0s;strokeBox.end"
            />
          </rect>
        </svg>
      </div>
    </div>
  );
}

export default PreLoader;
